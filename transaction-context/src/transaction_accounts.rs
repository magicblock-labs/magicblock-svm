...
impl ReadableAccount for TransactionAccountView<'_> {
    fn lamports(&self) -> u64 {
        self.private_fields.account.lamports()
    }

    fn data(&self) -> &[u8] {
        self.private_fields.account.data()
    }

    fn owner(&self) -> &Pubkey {
        self.private_fields.account.owner()
    }

    fn executable(&self) -> bool {
        self.private_fields.account.executable()
    }

    fn rent_epoch(&self) -> u64 {
        self.private_fields.account.rent_epoch()
    }
}
...
impl TransactionAccountViewMut<'_> {
    ...
    pub(crate) fn set_data_from_slice(&mut self, new_data: &[u8]) {
        self.private_fields.account.set_data_from_slice(new_data);
        // SAFETY: We are synchronizing the lengths.
        unsafe {
    ...
    pub(crate) fn extend_from_slice(&mut self, data: &[u8]) {
        self.private_fields.account.extend_from_slice(data);
        // SAFETY: We are synchronizing the lengths.
        unsafe {
            self.abi_account
                .payload
    ...
    pub(crate) fn reserve(&mut self, additional: usize) {
        self.private_fields.account.reserve(additional);
    }
...
impl WritableAccount for TransactionAccountViewMut<'_> {
    fn set_lamports(&mut self, lamports: u64) {
        ...
        self.private_fields.account.set_lamports(lamports);
    }

    fn data_as_mut_slice(&mut self) -> &mut [u8] {
        self.private_fields.account.data_as_mut_slice()
    }

    fn set_owner(&mut self, owner: Pubkey) {
        self.abi_account.owner = owner;
        self.private_fields.account.set_owner(owner);
    }
    ...
    fn set_executable(&mut self, executable: bool) {
        self.private_fields.account.set_executable(executable);
    }

    fn set_rent_epoch(&mut self, epoch: u64) {
        self.private_fields.account.set_rent_epoch(epoch);
    }
